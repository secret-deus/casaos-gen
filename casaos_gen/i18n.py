...
def wrap_multilang(
    ...
    result: Dict[str, str] = {"en_US": english}
    text = english or ""
    is_long = len(text) > 60 or "\n" in text
...
def apply_multilang_services(
    ...
    for name, svc in meta.services.items():
        ...
        for env in svc.envs:
            svc_block["envs"].append(
                {
                    "container": env.container,
                    "description": wrap_multilang(env.description, languages, translations),
                }
            )
        for port in svc.ports:
            svc_block["ports"].append(
                {
                    "container": port.container,
                    "description": wrap_multilang(port.description, languages, translations),
                }
            )
        for vol in svc.volumes:
            svc_block["volumes"].append(
                {
                    "container": vol.container,
                    "description": wrap_multilang(vol.description, languages, translations),
                }
            )
